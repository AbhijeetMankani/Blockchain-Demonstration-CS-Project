...
def mine_transaction(T_ID):
	...
	if (sender_id == 'BLOCK_MINE_REWARD' and not mined):
		...
		query = '''SELECT Rewards_Won FROM Users WHERE Public_Key="{Public_Key}";'''.format(Public_Key = receiver_id)
		cursor.execute(query)
		RW_Receiver = cursor.fetchone()[0] + 1

		query = '''UPDATE Users SET Balance = {NewBal} WHERE Public_Key = "{Receiver_ID}";'''.format(NewBal = NewBal_Receiver, Receiver_ID = receiver_id)
		cursor.execute(query)

		query = '''UPDATE Users SET Rewards_Won = {RW} WHERE Public_Key = "{Receiver_ID}";'''.format(RW = RW_Receiver, Receiver_ID = receiver_id)
		cursor.execute(query)
		...
		try:
			...
			print("Invalid Transaction - Invlid/Corrupt Signature")
			return
...
def send_reward(Block_ID, User_ID, Amount):
	...
	transaction = "{" + '''"Transaction_ID": {T_ID}, "sender_id": "BLOCK_MINE_REWARD", "receiver_id": "{receiver_id}", "amount": "{amount}"'''.format(T_ID = T_ID, receiver_id = User_ID, amount=Amount) + "}"

	signature = hashlib.sha256(transaction.encode()).hexdigest()

	query = '''INSERT INTO All_Transactions VALUES ("{T_ID}", "BLOCK_MINE_REWARD", "{Reciever_ID}", {Amount}, FALSE, '{transaction}', "{sign}");'''.format(T_ID = T_ID, Reciever_ID = User_ID, Amount = Amount, transaction = transaction, sign = "BLOCK_MINE_REWARD_" + signature)
	cursor.execute(query)
	query = '''INSERT INTO Unmined_Transactions VALUES ("{T_ID}", "BLOCK_MINE_REWARD", "{Reciever_ID}", {Amount});'''.format(T_ID = T_ID, Reciever_ID = User_ID, Amount = Amount)
	cursor.execute(query)
